...
impl PodmanComputeDriver {
    ...
    pub async fn stop_sandbox(&self, sandbox_id: &str) -> Result<(), ComputeDriverError> {
        let name = self
            .find_container_name_by_id(sandbox_id)
            .await?
            .ok_or_else(|| {
                ComputeDriverError::Precondition("sandbox container not found".into())
            })?;
        info!(sandbox_id = %sandbox_id, container = %name, "Stopping sandbox container");
    ...
    pub async fn delete_sandbox(&self, sandbox_id: &str) -> Result<bool, ComputeDriverError> {
        ...
        let Some(name) = self.find_container_name_by_id(sandbox_id).await? else {
        ...
        info!(sandbox_id = %sandbox_id, container = %name, "Deleting sandbox container");
        ...
        // list and here (TOCTOU race); proceed with volume cleanup
...
mod tests {
    ...
    async fn delete_sandbox_finds_container_by_label_and_removes() {
        ...
        let requests = request_log
            ...
            .clone();
        assert!(requests[0].contains("/libpod/containers/json"));
        assert!(requests[1].contains(&format!("/libpod/containers/{container_name}/stop")));
        assert!(requests[2].contains(&format!("/libpod/containers/{container_name}")));
        assert_eq!(
            requests[3],
            format!(
                "DELETE {}",
