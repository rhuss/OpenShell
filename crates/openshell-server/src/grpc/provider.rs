...
pub(super) async fn create_provider_record(
    ...
    if provider.credentials.is_empty()
        && !provider_type_allows_empty_credentials(store, workspace, &provider.r#type).await?
    {
...
pub(super) async fn update_provider_record_with_catalog(
    ...
    validate_provider_update_against_attached_sandboxes(store, workspace, &candidate).await?;
...
async fn scan_sandboxes<T, F>(store: &Store, workspace: &str, mut f: F) -> Result<Vec<T>, Status>
where
    F: FnMut(Sandbox) -> Option<T>,
{
    ...
    loop {
        let records = store
            .list(Sandbox::object_type(), workspace, 1000, offset)
            .await
...
pub(super) async fn resolve_provider_environment_with_catalog(
    ...
    validate_provider_environment_keys_unique_at(store, workspace, provider_names, None, now_ms)
        .await?;
    ...
    Ok(ProviderEnvironment {
        ...
        dynamic_credentials: resolve_dynamic_credentials(store, workspace, provider_names).await?,
    })
}
...
pub(super) async fn resolve_dynamic_credentials_with_catalog(
    ...
    for provider_name in provider_names {
        ...
        let Some(profile) = get_provider_type_profile(store, workspace, profile_id).await? else {
            continue;
        };
...
pub(super) async fn handle_create_provider(
    ...
    let result = create_provider_record(state.store.as_ref(), &workspace, provider).await;
    match result {
        Ok(provider) => {
            emit_provider_lifecycle(
                &provider.r#type,
...
pub(super) async fn handle_update_provider_profiles(
    ...
    let catalog = state
        ...
        .await?;
    diagnostics.extend(
        profile_update_target_diagnostics(state.store.as_ref(), &workspace, &profiles, &target_id)
            .await?,
    );
    diagnostics.extend(validate_profile_set(&profiles));
...
async fn provider_refresh_defaults(
    ...
    let Some(profile) = get_provider_type_profile(store, workspace, &provider.r#type).await? else {
        return Ok(None);
    };
    Ok(profile
        .credentials
...
async fn provider_type_allows_empty_credentials(
    ...
    let Some(profile) = get_provider_type_profile(store, workspace, provider_type).await? else {
        return Ok(false);
    };
...
async fn merged_provider_profiles(
    store: &Store,
    workspace: &str,
) -> Result<Vec<ProviderTypeProfile>, Status> {
    let mut profiles = default_profiles().to_vec();
    profiles.extend(
        custom_provider_profiles(store, workspace)
            .await?
            .into_iter()
            .filter_map(|stored| {
                let resource_version = stored_profile_resource_version(&stored);
...
async fn custom_provider_profiles(
    store: &Store,
    workspace: &str,
...
async fn profile_attached_sandbox_diagnostics(
    ...
    let sandboxes = scan_sandboxes(store, workspace, |sandbox| {
        sandbox
            .spec
            .as_ref()
...
async fn sandboxes_using_profile(
    ...
    let candidates = scan_sandboxes(store, workspace, |sandbox| {
...
pub(super) async fn handle_update_provider(
    ...
    let result = update_provider_record(state.store.as_ref(), &workspace, provider).await;
    match result {
        Ok(provider) => {
            emit_provider_lifecycle(
                &provider.r#type,
...
pub(super) async fn handle_configure_provider_refresh(
    ...
    if let Some(expires_at_ms) = request.expires_at_ms {
        ...
        update_provider_record(state.store.as_ref(), &workspace, updated).await?;
    }
